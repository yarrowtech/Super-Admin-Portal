...
exports.getTodayAttendanceStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...attendance.toObject(),
        checkedIn: Boolean(attendance.checkIn && !attendance.checkOut)
      }
    });
  } catch (error) {
    logger.error({ err: error }, 'Get today attendance status error');
    return res.status(500).json({
      success: false,
      error: 'Failed to fetch today attendance status',
      details: error.message
    });
  }
};
