...
const emitLegalEvent = (req, event, payload) => {
  const io = req.app?.get('io');
  if (io) io.emit(event, payload);
};

const createAuditLog = async (req, documentId, action, remarks = '', metadata = {}) => {
...
const requirePuppeteer = () => {
  try {
    ...
    const err = new Error('PDF generation requires puppeteer to be installed');
    err.statusCode = 500;
    throw err;
  }
};
...
const buildPdfHtml = (doc) => {
  ...
  const content = doc.latestContent || '<p>No content available.</p>';
  const remarks = doc.ceoRemarks
    ? `<section class="meta-block"><h3>CEO Remarks</h3><p>${doc.ceoRemarks}</p></section>`
    : '';
  ...
  <html>
    <head>
      ...
      <style>
        ...
        .meta { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 24px; font-size: 13px; color: #374151; }
        .meta strong { color: #111827; }
        .content { font-size: 14px; line-height: 1.6; word-break: break-word; }
        .meta-block { margin-top: 24px; padding: 16px; border: 1px solid #d1d5db; border-radius: 10px; background: #f9fafb; }
        .meta-block h3 { margin: 0 0 8px; font-size: 15px; }
    ...
    <body>
      <div class="page">
        <header>
          <h1>${title}</h1>
          <div class="meta">
            <div><strong>Type:</strong> ${doc.type || 'Other'}</div>
            <div><strong>Version:</strong> ${doc.currentVersion || 'v1.0'}</div>
            <div><strong>Status:</strong> ${doc.status || 'Draft'}</div>
            <div><strong>Project:</strong> ${doc.projectName || 'N/A'}</div>
            <div><strong>Owner:</strong> ${doc.owner || 'N/A'}</div>
            <div><strong>Updated:</strong> ${doc.updatedAt ? new Date(doc.updatedAt).toISOString() : 'N/A'}</div>
          </div>
        </header>
...
const createVersionSnapshot = async (doc, req, summary) => {
  const actor = getActorContext(req);
  const content = doc.latestContent && String(doc.latestContent).trim() ? doc.latestContent : '<p><br></p>';
...
exports.create = async (req, res) => {
  try {
    ...
    await LegalDocumentVersion.create({
      documentId: doc._id,
      version: 'v1.0',
      content: content && String(content).trim() ? content : '<p><br></p>',
      editedBy: actor.actorId,
      editedByName: actor.name,
    ...
    await createAuditLog(req, doc._id, 'CREATE', 'Document created', {
      title: doc.title,
      type: doc.type,
      version: doc.currentVersion,
    ...
    emitLegalEvent(req, 'legal:document:created', {
      documentId: doc._id,
      title: doc.title,
...
exports.autoSave = async (req, res) => {
  try {
    ...
    if (req.body.type) doc.type = VALID_TYPES.has(req.body.type) ? req.body.type : doc.type;
...
exports.saveDraft = async (req, res) => {
  try {
    ...
    await createVersionSnapshot(doc, req, changeSummary || 'Draft saved');
    await createAuditLog(req, doc._id, 'UPDATE', changeSummary || 'Draft saved', {
      version: doc.currentVersion,
      status: doc.status,
    });

    emitLegalEvent(req, 'legal:document:updated', {
      documentId: doc._id,
      title: doc.title,
...
exports.submit = async (req, res) => {
  try {
    ...
    if (!doc.latestContent || !String(doc.latestContent).trim()) {
      return res.status(400).json({ success: false, error: 'Content required before submission' });
    }
    ...
    emitLegalEvent(req, 'legal:document:submitted', {
      documentId: doc._id,
      title: doc.title,
...
exports.myDocuments = async (req, res) => {
  try {
    ...
    const [items, total] = await Promise.all([
      LegalDocument.find(filter).sort({ updatedAt: -1 }).skip(skip).limit(normalizedLimit).lean(),
      LegalDocument.countDocuments(filter),
    ]);

    return res.json({
      success: true,
      data: { items, total, page: normalizedPage, limit: normalizedLimit },
    });
  } catch (err) {
...
exports.getPending = async (req, res) => {
  try {
    ...
    const skip = (normalizedPage - 1) * normalizedLimit;
    const [items, total] = await Promise.all([
      LegalDocument.find(filter).sort({ submittedAt: -1 }).skip(skip).limit(normalizedLimit).lean(),
      LegalDocument.countDocuments(filter),
    ]);
...
exports.approve = async (req, res) => {
  try {
    ...
    doc.approvedBy = actor.actorId;
    doc.approvedAt = new Date();
    doc.rejectedAt = null;
    doc.ceoRemarks = req.body.remarks || '';

    await doc.save();
    await createAuditLog(req, doc._id, 'APPROVE', req.body.remarks || 'Approved', {
      approvedAt: doc.approvedAt,
      version: doc.currentVersion,
    });

    emitLegalEvent(req, 'legal:document:approved', {
      documentId: doc._id,
      title: doc.title,
      approvedBy: actor.name,
      version: doc.currentVersion,
      remarks: doc.ceoRemarks,
    });
...
exports.reject = async (req, res) => {
  try {
    ...
    await doc.save();
    await createAuditLog(req, doc._id, 'REJECT', remarks, {
      rejectedAt: doc.rejectedAt,
      version: doc.currentVersion,
    });

    emitLegalEvent(req, 'legal:document:rejected', {
      documentId: doc._id,
      title: doc.title,
...
exports.restoreVersion = async (req, res) => {
  try {
    ...
    if (String(version.documentId) !== String(doc._id)) {
      return res.status(400).json({ success: false, error: 'Version does not belong to this document' });
    }
    ...
    await createVersionSnapshot(doc, req, `Restored from ${version.version}`);
    await createAuditLog(req, doc._id, 'RESTORE', `Restored from ${version.version}`, {
      restoredFrom: version._id,
      version: doc.currentVersion,
    });

    emitLegalEvent(req, 'legal:document:restored', {
      documentId: doc._id,
      title: doc.title,
...
exports.deleteDocument = async (req, res) => {
  try {
    ...
    await createAuditLog(req, doc._id, 'DELETE', 'Document deleted', {
      status: doc.status,
      version: doc.currentVersion,
    });
...
exports.generatePdf = async (req, res) => {
  try {
    ...
    try {
      ...
      await page.setContent(buildPdfHtml(doc), { waitUntil: 'load' });
      ...
      await createAuditLog(req, doc._id, 'PDF_GENERATE', 'PDF generated', {
        version: doc.currentVersion,
        status: doc.status,
      });

      const safeTitle =
        String(doc.title || 'legal-document')
          .trim()
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, '-')
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="${safeTitle}-${doc.currentVersion || 'v1-0'}.pdf"`
      );
