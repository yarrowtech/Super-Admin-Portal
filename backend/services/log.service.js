...
const normalizePayload = (input = {}) => {
  ...
  const doc = {
    ...
    status: cleanString(input.status || input.metadata?.status, 80),
    requestId: cleanString(input.requestId, 160),
    sessionId: cleanString(input.sessionId, 160),
    portal: cleanString(input.portal, 160),
    page: cleanString(input.page, 200),
    frontendRoute: normalizeRoute(input.frontendRoute),
    ...
    entityType: cleanString(input.entityType, 160),
...
const create = async (input = {}) => {
  ...
  const pinoPayload = {
    ...
    portal: doc.portal,
    page: doc.page,
    frontendRoute: doc.frontendRoute,
    method: doc.method,
    route: doc.route,
...
const createFromRequest = (req, input = {}) => {
  ...
  return create({
    ...
    sessionId: input.sessionId || (req?.authSessionId ? String(req.authSessionId) : undefined),
    method: req?.method,
    route,
