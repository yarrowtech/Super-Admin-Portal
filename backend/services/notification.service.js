...
const ManagerNotification = require('../models/manager/ManagerNotification');
...
const findTargetManagers = async (department) => {
  ...
  [...managersByDept, ...(managersByDept.length ? [] : fallbackManagers), ...ceoUsers].forEach(
    (manager) => {
      if (!manager || !manager._id) return;
      unique.set(manager._id.toString(), manager);
    }
  );
