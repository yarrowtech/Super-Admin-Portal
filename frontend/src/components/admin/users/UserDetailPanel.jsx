...
const accessLevelMap = {
  ...
  elevated: 'Elevated / project admin',
...
const UserDetailPanel = ({
  ...
  if (!user) {
    return (
      <div className="flex h-full min-h-[420px] items-center justify-center rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
  ...
  const formattedStartDate =
    metadata.startDate && !Number.isNaN(new Date(metadata.startDate).getTime())
      ...
      : null;
  const employmentLabel = employmentTypeMap[metadata.employmentType];
  const workModeLabel = workModeMap[metadata.workMode];
  const accessLevelLabel = accessLevelMap[metadata.accessLevel];
  const hasPositionProfile = metadata.positionFocus || metadata.teamName || positionLevelLabel;
  const hasWorkPreferences = employmentLabel || workModeLabel || formattedStartDate || accessLevelLabel;

  return (
    <div className="flex h-full max-h-[78vh] min-h-[620px] flex-col gap-4 overflow-hidden rounded-2xl border border-neutral-200 bg-gradient-to-br from-white to-neutral-50 p-4 shadow-sm dark:border-neutral-800 dark:from-neutral-900 dark:to-neutral-800 lg:p-5">
      {/* Header with Avatar */}
      <div className="flex items-start gap-3 border-b border-neutral-200 pb-4 dark:border-neutral-800">
        <div className="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary via-primary to-primary/80 text-xl font-bold text-white shadow-md ring-2 ring-primary/20">
          {initials}
          {user.isActive && (
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{fullName}</h2>
            {user.isActive ? (
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pr-1">
        ...
        {hasPositionProfile && (
          <div>
            ...
            <div className="space-y-3">
              {metadata.positionFocus && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">workspace_premium</span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Focus</p>
                    <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{metadata.positionFocus}</p>
                  </div>
                </div>
              )}
              {positionLevelLabel && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">
                    {metadata.positionLevel === 'team_lead' ? 'military_tech' : 'badge'}
                  </span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Position Level</p>
                    <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{positionLevelLabel}</p>
                  </div>
                </div>
              )}
              {metadata.teamName && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">groups</span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Team</p>
                    <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{metadata.teamName}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        {hasWorkPreferences && (
          <div>
            ...
            <div className="space-y-3">
              {employmentLabel && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">work_history</span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Employment Type</p>
                    <p className="text-sm font-semibold capitalize text-neutral-800 dark:text-neutral-200">{employmentLabel}</p>
                  </div>
                </div>
              )}
              {workModeLabel && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">home_work</span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Work Mode</p>
                    <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{workModeLabel}</p>
                  </div>
                </div>
              )}
              {formattedStartDate && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">event</span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Start Date</p>
                    <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{formattedStartDate}</p>
                  </div>
                </div>
              )}
              {accessLevelLabel && (
                <div className="flex items-start gap-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
                  <span className="material-symbols-outlined text-neutral-600 dark:text-neutral-400">verified_user</span>
                  <div>
                    <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Access Level</p>
                    <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{accessLevelLabel}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        {skills.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2 rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3">
              {skills.map((skill) => (
                <span
                  key={skill}
        ...
        {metadata.notes && (
          <div>
            ...
            <div className="rounded-lg bg-neutral-50 dark:bg-neutral-800/60 p-3 text-sm text-neutral-700 dark:text-neutral-200">
              {metadata.notes}
            </div>
          </div>
        )}
