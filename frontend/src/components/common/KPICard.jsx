...
const KPICard = ({
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border-2 bg-gradient-to-br ${colors.gradient} ${compact ? 'p-4 lg:p-5' : 'p-6'} shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-105 hover:${colors.border} ${className}`}
    >
      ...
      <div className="relative">
        <div className={`${compact ? 'mb-3' : 'mb-4'} flex items-center justify-between`}>
          <div
            className={`flex ${compact ? 'h-11 w-11 rounded-xl' : 'h-14 w-14 rounded-2xl'} items-center justify-center bg-gradient-to-br ${colors.iconBg} text-white shadow-lg ring-4 ${colors.iconRing}`}
          >
        ...
        <p className={`mb-1 ${compact ? 'text-3xl lg:text-4xl' : 'text-5xl'} font-black tracking-tight text-neutral-900 dark:text-neutral-100`}>{value}</p>
