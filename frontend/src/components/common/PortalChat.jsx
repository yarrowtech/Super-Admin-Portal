...
const PortalChat = ({
  ...
  api = employeeApi,
  fetchTeamMembers,
  teamMembersExtractor = defaultTeamMembersExtractor,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.getChatThreads(token);
        const list = res?.data || res || [];
        const normalized = list.map(normalizeThread);
        setThreads(normalized);
        if (normalized.length > 0 && !activeThreadIdRef.current) {
          ...
          if (savedThreadExists) {
            ...
            activeThreadIdRef.current = savedThreadId;
          } else {
  ...
  }, [token, normalizeThread, getThreadId, storageKey, api]);

  useEffect(() => {
    if (!token) return;

    const fetcher = fetchTeamMembers || api.getTeam;
    if (!fetcher) return;

    (async () => {
      try {
        const res = await fetcher(token);
        const rawMembers = teamMembersExtractor(res) || [];
        const normalizedMembers = rawMembers
          .map((member) => {
            const rawId =
              ...
              member?._id?.toString?.() ||
              null;
            const normalizedId = rawId?.toString?.() || rawId || null;
            const computedName =
              member?.name ||
            ...
            return {
              ...
              _id: member?._id || normalizedId,
              name: member?.name || computedName,
            };
          })
          .filter((member) => {
  ...
  const loadMessages = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await api.getChatMessages(token, activeThreadId);
  ...
  }, [token, activeThreadId, getThreadId, api]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      const response = await api.postChatMessage(token, activeThreadId, messageText);
  ...
  const selectQuickReply = async (reply) => {
    ...
    try {
      await api.postChatMessage(token, activeThreadId, reply);
  ...
  const handleStartChat = async (member) => {
    ...
    setError('');
    try {
      const res = await api.createChatThread(token, member.id);
  ...
  const handleCreateGroup = async ({ name, memberIds, meta }) => {
    ...
    try {
      ...
      const res = await api.createGroupThread(token, payload);
      const thread = normalizeThread(res?.data || res);
      const threadId = getThreadId(thread);
      setThreads((prev) => {
