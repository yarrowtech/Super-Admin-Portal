...
const ChatWindow = ({
  ...
  const getMemberRoleLabel = (member) => {
    ...
    if (department) {
      return `${department.charAt(0).toUpperCase()}${department.slice(1)} team`;
    }
    return '';
  };
  ...
  const renderedMessages = messages.map((msg, index) => {
    ...
    const previousDateKey = previousMessage ? getDateKey(previousMessage.time) : null;
  ...
  const directPartner = !isGroupChat
    ? groupMembers.find((member) => {
        const memberId = member?.id || member?._id;
        if (!memberId) return false;
        const normalizedMemberId = memberId?.toString?.() || memberId;
        const normalizedCurrentId = currentUserId?.toString?.() || currentUserId;
        return normalizedMemberId !== normalizedCurrentId;
      }) || null
    : null;
  const directSubtitle = directPartner
    ? getMemberRoleLabel(directPartner)
