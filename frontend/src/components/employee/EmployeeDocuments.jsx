...
const EmployeeDocuments = () => {
  ...
  const onUploadFile = async (event) => {
    ...
    try {
      ...
      await employeeApi.uploadDocument(token, {
        title: file.name,
  ...
  const onDownloadDocument = async (doc) => {
    if (!token) return;
    try {
      const res = await employeeApi.downloadDocument(token, doc.id || doc._id);
      const payload = res?.data || res || {};
      const url = payload?.downloadUrl || payload?.url;
      if (!url) throw new Error('Download URL not available');
      const fileName = payload?.fileName || doc?.name || 'document';
    ...
    <main className="mx-auto flex max-w-[1400px] flex-col gap-6">
      <header className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-lg shadow-primary/5 dark:border-slate-800 dark:bg-slate-900/70">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:border-primary hover:text-primary dark:border-slate-700 dark:text-slate-200">
