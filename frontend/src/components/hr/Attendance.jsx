...
const Attendance = () => {
  ...
  const allVisibleSelected =
    ...
    filteredRecords.every((record) => selectedRecordIds.includes(record._id));
  ...
  const handleExport = async (scope) => {
    ...
    try {
      ...
      toast.success(scope === 'selected' ? `Exported ${selectedRecordIds.length} selected attendance rows.` : 'Attendance CSV exported successfully.');
                  ...
                  groupedRecords.map((group) => (
                    <React.Fragment key={group.key}>
                      <tr className="bg-neutral-50 text-xs font-semibold uppercase text-neutral-500 dark:bg-neutral-900 dark:text-neutral-400">
                        <td colSpan={9} className="p-2 pl-3">{group.label}</td>
                      </tr>
