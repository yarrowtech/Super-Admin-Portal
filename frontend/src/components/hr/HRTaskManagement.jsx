  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        progress: getProgressFromStatus(form.status, editingTask?.progress ?? 0),
      };
      ...
      if (!editingTask) {
        ...
        await hrApi.updateTask(taskId, { ...payload, status: form.status }, token);
      }
  ...
  const formattedTasks = useMemo(() => {
    return tasks.map((task) => {
      ...
      return {
        ...
        progressValue: task.progress ?? getProgressFromStatus(task.status),
      };
    });
  }, [tasks]);
    ...
    >
      <div className="space-y-6">
          ...
          {[
            {
              ...
              description: 'Assign and control work items',
            },
            {
              key: 'updates',
              label: 'Work Updates',
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-black text-gray-900 dark:text-white">Operational Queue</h2>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Monitor assignments, close blockers, and rebalance workload quickly.
            </p>
          </div>
          <button
