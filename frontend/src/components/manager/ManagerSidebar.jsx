...
const ManagerSidebar = () => {
  ...
  return (
    <aside className="fixed left-0 top-0 z-50 h-screen w-[250px] overflow-hidden border-r border-gray-200 bg-white transition-transform -translate-x-full dark:border-gray-800 dark:bg-gray-900 md:translate-x-0">
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  key={item.path}
                  to={item.path}
                  className={`flex min-h-11 items-center gap-3 rounded-lg border-l-2 px-3 py-2.5 text-sm font-medium transition-colors ${
                    isActive
                      ? 'border-primary bg-primary/10 text-primary dark:bg-primary/20'
                      : 'border-transparent text-gray-800 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800/50'
                  }`}
                >
                  <span
