...
/** Read-only "Linked items" for a Law task. Server-side access is by task assignment + link. */
const LinkedItemsSection = ({ taskId, hasLinks }) => {
  const { token } = useAuth();
  ...
  return (
    <div>
      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-neutral-400">Linked items</p>
      {!hasLinks ? (
              ...
              <li key={key} className="rounded-xl border border-neutral-200 dark:border-neutral-700">
                <button type="button" disabled={item.missing} onClick={() => setOpenKey(open ? null : key)} className="flex w-full items-center gap-3 px-3 py-2 text-left disabled:opacity-60">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-xs text-neutral-500">{item.missing ? 'No longer available' : `${MODULE_LABELS[item.module] || 'Item'} · ${humanize(item.type)}${item.fileCount ? ` · ${item.fileCount} file${item.fileCount === 1 ? '' : 's'}` : ''}`}</span>
                  </span>
