...
export const hrApi = {
  ...
  getPredictiveAlerts: async (token) => {
    return apiClient.get('/api/analytics/hr/predictive-alerts', token);
  },
  getAutomationOverview: async (token) => {
    return apiClient.get('/api/automation/hr/overview', token);
  },
  getHrReportsOverview: async (token) => {
  ...
  exportAttendanceCsv: async ({ token, status, startDate, endDate, selectedIds = [] }) => {
    ...
    const query = params.toString();
    const response = await fetch(`${API_BASE_URL}/api/dept/hr/attendance/export${query ? `?${query}` : ''}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      ...
      throw new Error(payload?.error || payload?.message || 'Failed to export attendance');
    }

    const disposition = response.headers.get('Content-Disposition') || '';
    const match = disposition.match(/filename="([^"]+)"/i);
    const fileName = match?.[1] || `hr-attendance-export-${Date.now()}.csv`;
    const blob = await response.blob();
    return { blob, fileName };
  },
  getAttendanceExportHistory: async (token, params = {}) => {
    const query = buildQueryString(params);
    return apiClient.get(`/api/dept/hr/attendance/export-history${query ? `?${query}` : ''}`, token);
  },
  getAttendanceStatus: async (token) => {
    return apiClient.get('/api/attendance', token);
  },
  ...
  getLeaveBalances: async (token, params = {}) => {
    const query = new URLSearchParams(params).toString();
    return apiClient.get(`/api/dept/hr/leave/balances${query ? `?${query}` : ''}`, token);
  },
  ...
  getPerformanceSystemOverview: async (token, params = {}) => {
    const query = buildQueryString(params);
    return apiClient.get(`/api/dept/hr/performance-system/overview${query ? `?${query}` : ''}`, token);
  },
  getPerformanceSystemEmployee: async (employeeId, token, params = {}) => {
    const query = buildQueryString(params);
    return apiClient.get(`/api/dept/hr/performance-system/employees/${employeeId}${query ? `?${query}` : ''}`, token);
  },
  recalculatePerformanceSnapshot: async (employeeId, data, token) => {
    return apiClient.post(`/api/dept/hr/performance-system/employees/${employeeId}/recalculate`, data, token);
  },
  getPerformanceSystemSnapshots: async (token, params = {}) => {
    const query = buildQueryString(params);
    return apiClient.get(`/api/dept/hr/performance-system/snapshots${query ? `?${query}` : ''}`, token);
  },
  getPerformanceSystemAppraisalCycles: async (token, params = {}) => {
    const query = buildQueryString(params);
    return apiClient.get(`/api/dept/hr/performance-system/appraisal-cycles${query ? `?${query}` : ''}`, token);
  },
  createPerformanceSystemAppraisalCycle: async (data, token) => {
    return apiClient.post('/api/dept/hr/performance-system/appraisal-cycles', data, token);
  },
